<template>
     <div class="fly-search-container060">
        ...
        <div class="fly-search-container095">
          ...
          <!---------------------flightResult----------------------->
          <div
            v-for="flight in flightResults.flightsData"
            :key="flight.flightBufferReferenceId"
          >
            <!---------- Info --------->
            <FlySearchShowResultsInfo
              :flightLeg="flight"
...
const searchInfo = () => {
  ...
  $fetch("https://marketplace.beta.luxota.network/v1/search/info", {
    ...
    .then((response) => {
      ...
      flightResults.updateOriginAirportTitle(
        response.legs[0].origin.buffer.title
      );
      flightResults.updateOriginAirportId(response.legs[0].origin.buffer.id);

      flightResults.updateDestAirportTitle(
        response.legs[0].destination.buffer.title
      );
      ...
      if (flightFields.tripType == "roundTrip") {
        ...
        flightResults.updateDepartureDateReturnToSend(
          response.legs[1].departure
        );
      }
